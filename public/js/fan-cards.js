(async function () {
  ...
  function renderLoading() {
    list.innerHTML = '<article class="fan-card"><p class="fan-message">팬 카드 데이터를 불러오는 중입니다.</p></article>';
    $('fanCount').textContent = '팬 카드를 불러오는 중입니다.';
    renderReadQueue([]);
  }

  function renderEmpty() {
    list.innerHTML = '<article class="fan-card"><p class="fan-message">표시할 팬 카드가 없습니다.</p></article>';
    $('fanCount').textContent = '표시할 팬 카드가 없습니다.';
  }

  function renderError(error) {
    list.innerHTML = `<article class="fan-card"><p class="fan-message">${api.escapeHtml(error?.message || '팬 카드 데이터를 불러오지 못했습니다.')}</p></article>`;
    $('fanCount').textContent = '팬 카드 데이터를 불러오지 못했습니다.';
    renderReadQueue([]);
  }
  ...
  function renderCard(item) {
    ...
    const date = item.createdAt ? new Date(item.createdAt).toLocaleString('ko-KR') : '날짜 없음';
    return `
      <article class="fan-card" data-fan-id="${api.escapeHtml(item.id)}" data-fan-status="${api.escapeHtml(status)}">
        <div class="fan-card-head">
          <div class="fan-author">
            <div class="fan-avatar">${api.escapeHtml(Array.from(item.senderName || '익명')[0] || '?')}</div>
            <div><strong>${api.escapeHtml(item.senderName || '익명 팬')}</strong><span>${api.escapeHtml(date)} · ${api.escapeHtml(status)}</span></div>
          </div>
          <span class="pill ${statusPill(status)}">${api.escapeHtml(status)}</span>
        </div>
        <p class="fan-message">${api.escapeHtml(item.message || '내용 없는 팬 카드')}</p>
        <div class="fan-actions">
          ${status !== 'APPROVED' ? '<button class="ghost-btn" data-status="APPROVED">공개</button>' : ''}
          ${status !== 'HIDDEN' ? '<button class="ghost-btn" data-status="HIDDEN">숨김</button>' : ''}
          ${status !== 'PENDING' ? '<button class="ghost-btn" data-status="PENDING">신규로 이동</button>' : ''}
          <button class="danger-btn" data-delete>삭제</button>
        </div>
      </article>
    `;
  }
  ...
  function renderReadQueue(items = []) {
    ...
    if (badge) badge.textContent = `${approved.length}개`;
    if (!approved.length) {
      queue.innerHTML = '<div class="list-item"><div class="item-content"><strong>공개 승인된 팬 카드가 없습니다.</strong></div></div>';
      return;
    }
    queue.innerHTML = approved.map((item) => `
      <div class="list-item">
        ...
        <div class="item-content">
          <strong>${api.escapeHtml(item.message || '내용 없는 팬 카드')}</strong>
          <span>${api.escapeHtml(item.senderName || '익명 팬')}</span>
        </div>
      </div>
    `).join('');
  }
  ...
  function render(data, allData) {
    ...
    $('fanCount').textContent = `${numberText(data.total)}개의 팬 카드 중 ${numberText(lastItems.length)}개를 표시 중입니다.`;
  ...
  async function approveAllPending() {
    ...
    await load();
    api.showToast(`${pending.length}개의 신규 팬 카드를 공개 처리했습니다.`);
  }
  ...
  list.addEventListener('click', async (event) => {
    ...
    try {
      ...
      if (status) {
        ...
        api.showToast('팬 카드 상태가 변경되었습니다.');
      }
      await load();
    } catch (error) {
